...
export default function QuizPage() {
  ...
  // ✅ Guard in case quizQuestions is undefined or empty
  const safeQuestions = Array.isArray(quizQuestions) ? quizQuestions : [];
  const totalQuestionSteps = safeQuestions.length;

  const extraSteps = 3; // intensity + budget + persona
  ...
  return (
    <>
      <div className="max-w-4xl mx-auto px-4 py-6 md:py-10">
        ...
        {/* If for some reason quizQuestions is empty, don't crash */}
        {totalQuestionSteps === 0 ? (
          <div className="text-sm text-slate-300">
            Quiz questions are not configured yet.
          </div>
        ) : (
          <motion.main
            ...
            <section className="max-w-3xl mx-auto space-y-5">
              <div className="space-y-2 text-center">
                ...
                <p className="text-sm md:text-base text-slate-400">
                  We&apos;ll translate your mood, lifestyle and aesthetics into
                  3 niche perfume recommendations — explained in emotion-first
                  language.
                </p>
              </div>
                  ...
                  {step < totalQuestionSteps && (
                    <>
                      <h2 className="text-base md:text-lg text-slate-50 mb-4">
                        {safeQuestions[step].question}
                      </h2>
                      <div className="grid md:grid-cols-2 gap-3">
                        {safeQuestions[step].options.map((option: string) => (
                          <motion.button
                            key={option}
                            whileTap={{ scale: 0.97 }}
              ...
              <p className="text-[0.7rem] text-slate-500 text-center">
                Your choices, intensity, budget and vibe description feed
                directly into the AI recommendation engine behind the Results
                page.
              </p>
            </section>
          </motion.main>
